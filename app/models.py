...
class Week(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint('season_id', 'week_number', name='uq_season_week'),
    )

    @property
    def is_last_or_second_to_last(self):
        total = self.season.total_weeks
        return self.week_number >= total - 1
...
class Game(db.Model):
    ...
    def has_started(self):
        if self.game_time is None:
            return False
        return datetime.now(timezone.utc) >= self.game_time

    @property
    def spread_display(self):
        if self.spread is None:
            return "N/A"
        spread_val = abs(self.spread)
        if self.favorite == 'home':
            return f"{self.home_team} -{spread_val}"
        else:
            return f"{self.away_team} -{spread_val}"
    ...
    def calculate_points(self, picked_team):
        ...
        if self.favorite == 'home':
            ...
            actual_margin = self.away_score - self.home_score
        fav_points = actual_margin - spread_val
        if picked_team == self.favored_team:
            raw_points = fav_points
        else:
            raw_points = -fav_points
        return max(-15, min(15, raw_points))
...
class PickViewLog(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint('user_id', 'week_id', name='uq_user_week_view'),
    )
...
class WeeklyResult(db.Model):
    ...
    __table_args__ = (
        db.UniqueConstraint('user_id', 'week_id', name='uq_user_week_result'),
    )
